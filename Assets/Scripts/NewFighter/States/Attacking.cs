...
public class Attacking : FighterState
{
    ...
    {        
        ...
        {
            // Update hurtboxes (Added Null Check)
            fighter.ClearHurtboxes();
            if (fighter.currentAction.hurtboxes != null)
            {
                foreach (HurtboxData data in fighter.currentAction.hurtboxes)
                {
                    ...
                    {
                        var availableHurtbox = fighter.GetAvailableHurtbox();
                        if (availableHurtbox != null) // Added Null Check
                        {
                            if (data.useBaseCollider)
                            {
                                fighter.standingHurtbox.Center = fighter.transform.position;
                                availableHurtbox.Init(Vector2.zero, fighter.standingHurtbox.Extents * 2f);
                            }
                            else
                            {
                                Vector2 offset = new Vector2(data.offset.x * (fighter.IsOnLeftSide ? 1 : -1), data.offset.y);
                                availableHurtbox.Init(offset, data.size);
                            }
                        }
                    }
                }
            ...
            // Update hitboxes (Added Null Check)
            fighter.ClearHitboxes();
            if (!fighter.actionHasHit && fighter.currentAction.hitboxes != null)
            {
                foreach (HitboxData data in fighter.currentAction.hitboxes)
                {
                    ...
                    {
                        Vector2 offset = new Vector2(data.offset.x * (fighter.IsOnLeftSide ? 1 : -1), data.offset.y);
                        var availableHitbox = fighter.GetAvailableHitbox();
                        if (availableHitbox != null) // Added Null Check
                        {
                            availableHitbox.Init(offset, data.size);
                        }
                    }
                }
            }

            // Movements (Added Null Check)
            if (fighter.currentAction.movements != null)
            {
            ...
            // Projectiles (Added Null Check)
            if (fighter.currentAction.projectiles != null)
            {
