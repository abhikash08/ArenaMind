...
public class NewFighter : MonoBehaviour
{
    ...
    private void Update()
    {
        if (!paused)
        {
            ...
            // --- THE FIX: Only let human input update the state machine ---
            if (!isAI)
            {
    ...
    {
        StopAllCoroutines(); 
        currentHealth = maxHealth; // <-- MOVED HERE: INSTANT REVIVAL
    ...
    { 
        if (col.gameObject.tag == "FighterBox" && col.transform.parent.GetComponent<NewFighter>() != this) actionHasHit = true; 
    }
    ...
    {
        foreach (NewCollisionBox hitbox in currentHitboxes) { if (!hitbox.gameObject.activeInHierarchy) return hitbox; }
        return null;
    }

    public bool HasActiveHitboxes()
    {
        foreach (NewCollisionBox hitbox in currentHitboxes) { if (hitbox.gameObject.activeInHierarchy) return true; }
        return false;
    }

    public NewCollisionBox GetAvailableHurtbox()
    {
        foreach (NewCollisionBox hurtbox in currentHurtboxes) { if (!hurtbox.gameObject.activeInHierarchy) return hurtbox; }
        return null;
    }
