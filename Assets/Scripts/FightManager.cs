...
public class FightManager : MonoBehaviour
{
    ...
    private void Update() { if (!hitstopActive) { UpdateSides(); Push(); } }
    ...
    private void LateUpdate()
    {
        if (!trainingMode && !mlAgentsTrainingMode && !roundOver)
        {
            if ((fighters[0].currentHealth > 0 && fighters[1].currentHealth <= 0) || 
                ...
                (fighters[0].currentHealth <= 0 && fighters[1].currentHealth <= 0))
            {
                Debug.Log($"[FightManager] K.O. DETECTED! P1 Health: {fighters[0].currentHealth} | P2 Health: {fighters[1].currentHealth}");
    ...
    private void ResetRound()
    {
        ...
        hitstopActive = false;
        roundOver = false;
        Debug.Log($"[FightManager] ROUND RESET. Bots fully revived for Round {roundNum}.");
    ...
    // --- FIX: NO MORE TIMER DURING ML TRAINING ---
    private IEnumerator StartRound()
    {
        ...
        // ONLY start the timer if a human is playing
    ...
    private IEnumerator Timer()
    {
        ...
        while (timeRemaining > 0) { for (int i = 0; i < 60; i++) yield return null; timeRemaining -= 1; timerText.text = timeRemaining.ToString(); }
    ...
    public void PlaySound(SoundType type, AudioSource source) {} // Audio logic handled elsewhere/muted
    ...
    public void OnFighterHit(NewFighter hitFighter, HitData hitData, bool attackWasBlocked)
    {
        ...
        if (!mlAgentsTrainingMode)
        {
            ...
            if (attackWasBlocked) Instantiate(blockParticlePrefab, particlePos, Quaternion.Euler(0f, hitFighter.IsOnLeftSide ? -66f : 66f, 0f));
            else Instantiate(hitParticlePrefab, particlePos, Quaternion.identity);
    ...
    private void OnTookDamage(NewFighter fighter)
    {
        if (fighter == fighters[0])
        {
            ...
            if (trainingMode && !mlAgentsTrainingMode) { if (regenCoroutines[0] != null) StopCoroutine(regenCoroutines[0]); regenCoroutines[0] = StartCoroutine(RegenHealth(0)); }
        }
        else if (fighter == fighters[1])
        {
            fighterHealthBars[1].fillAmount = Mathf.Max((float)fighter.currentHealth / fighter.maxHealth, 0f);
            if (trainingMode && !mlAgentsTrainingMode) { if (regenCoroutines[1] != null) StopCoroutine(regenCoroutines[1]); regenCoroutines[1] = StartCoroutine(RegenHealth(1)); }
        }
    }
    ...
    public void OnBreakThrow(NewFighter fighter, NewFighter opponent)
    {
        ...
        if (!mlAgentsTrainingMode) Instantiate(breakParticlePrefab, (fighter.transform.position + opponent.transform.position) / 2f, Quaternion.identity);
    ...
    private IEnumerator DelayedThrowInitialOffset(NewFighter fighter, Vector3 offset) { offset = !fighter.IsOnLeftSide ? offset : -offset; fighter.model.transform.Translate(fighter.model.transform.InverseTransformVector(offset)); yield return null; fighter.model.transform.Translate(fighter.model.transform.InverseTransformVector(-offset)); }

    private void UpdateSides()
    {
        if (fighters.Length != 2) return;

        // Allow them to turn around as long as they aren't actively attacking or getting hit
        bool canFlip0 = fighters[0].currentState is Walking || fighters[0].currentState is Falling;
        bool canFlip1 = fighters[1].currentState is Walking || fighters[1].currentState is Falling;

        if (fighters[0].transform.position.x - fighters[0].boxCollider.bounds.extents.x + 0.015f > fighters[1].transform.position.x) 
        { 
            if (canFlip0) fighters[0].SwitchSide(false, true); 
            if (canFlip1) fighters[1].SwitchSide(true, true); 
        }
        else if (fighters[0].transform.position.x + fighters[0].boxCollider.bounds.extents.x - 0.015f < fighters[1].transform.position.x) 
        { 
            if (canFlip0) fighters[0].SwitchSide(true, true); 
            if (canFlip1) fighters[1].SwitchSide(false, true); 
        }
    }    
    ...
    private void Push()
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            foreach (Collider2D col in horizontalHits) if (col.transform == fighters[1 - i].transform && Mathf.Sign(fighters[1 - i].boxCollider.bounds.center.x - fighters[i].boxCollider.bounds.center.x) != Mathf.Sign(fighters[1 - i].velocity.x)) fighters[i].controller.Move(fighters[1 - i].velocity.x * (1f / 60f) * Vector3.right);
                    ...
                    if (col.transform == fighters[1 - i].transform)
                    {
                        if (fighters[i].boxCollider.bounds.center.x > fighters[1 - i].boxCollider.bounds.center.x)
                        {
                            ...
                            if (overlap != 0)
                            {
                                ...
                                if (wallHit) fighters[i].controller.Move(1.5f * overlap * Vector3.right); else fighters[1 - i].controller.Move(1.5f * overlap * Vector3.left);
                            }
                            if (fighters[i].velocity.x < 0f) fighters[i].velocity = new Vector2(0f, fighters[i].velocity.y);
                        }
                        else if (fighters[i].boxCollider.bounds.center.x < fighters[1 - i].boxCollider.bounds.center.x)
                        {
                            ...
                            if (overlap != 0)
                            {
                                ...
                                if (wallHit) fighters[i].controller.Move(1.5f * overlap * Vector3.left); else fighters[1 - i].controller.Move(1.5f * overlap * Vector3.right);
                            }
                            if (fighters[i].velocity.x > 0f) fighters[i].velocity = new Vector2(0f, fighters[i].velocity.y);
                        }
                        else
                        {
                            ...
                            if (overlap != 0)
                            {
                                Vector3 raycastOrigin = fighters[i].IsOnLeftSide ? fighters[1 - i].boxCollider.bounds.center + Vector3.right * fighters[1 - i].boxCollider.bounds.extents.x * 0.9f : fighters[1 - i].boxCollider.bounds.center + Vector3.left * fighters[1 - i].boxCollider.bounds.extents.x * 0.9f;
                                ...
                                if (wallHit) fighters[1 - i].controller.Move(-1.5f * overlap * raycastDir); else fighters[1 - i].controller.Move(1.5f * overlap * raycastDir);
                            }
                        }
                    }
                }
